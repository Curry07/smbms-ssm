...
var idPicPathError = null;
var a_idPicPath = null;
var workPicPathError = null;
...
$(function(){
	...
	addBtn = $("#add");
	backBtn = $("#back");
	idPicPathError = $("#idPicPathError");
	a_idPicPath = $("#a_idPicPath");
	
	// workPicPathError = $("#workPicPathError");
	...
	a_idPicPath.next().html("*文件的大小必须限定在5M内，文件的格式只支持JPG,JPEG,PNG,PNEG!");
	// workPicPath.next().html("*文件的大小必须限定在5M内，文件的格式只支持JPG,JPEG,PNG,PNEG!");
	
	/*if(workPicPathError.val() !=""){
		workPicPath.next().html("*"+workPicPathError.val());
	}*/
	// 默认是空字符串
	if(idPicPathError.val() != ""){
		a_idPicPath.next().html("*"+idPicPathError.val());
	}
/*	$.ajax({
		type:"GET",//请求类型
	...
	userCode.bind("blur",function(){
		...
		/*$.ajax({
			type:"GET",//请求类型
			url:path+"/user/userCodeIsExists.html",//请求的url
	...
	// 验证每个表单元素的validateStatus属性(在你每一个表单元素的失去焦点时都会调用validateTip()函数 此函数即会帮你设置该信息)
