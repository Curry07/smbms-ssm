...
<%@include file="/WEB-INF/jsp/common/head.jsp"%>
        <div class="right">
            ...
            <div class="search">
           		<form method="post" action="${pageContext.request.contextPath }/auth/user/userlist">
					 <span>用户名：</span>
					 ...
					 <a href="${pageContext.request.contextPath}/auth/user/touseradd" >添加用户</a>
				</form>
            </div>
            <!--用户列表展示-->
            <table class="providerTable" cellpadding="0" cellspacing="0">
                <tr class="firstTr">
                   ...
                   <c:forEach var="user" items="${pageBean.result}" varStatus="status">
					<tr class="userView" userid=${user.id} >
						<td>
			...
			<input type="hidden" id="totalPageCount" value="${pageBean.totalPage}"/>
		  	<c:import url="rollpage.jsp">
	          	...
	          	<c:param name="currentPageNo" value="${pageBean.currentPageNo}"/>
	          	<c:param name="totalPageCount" value="${pageBean.totalPage}"/>
          	</c:import>
        
        <!--隐藏的分页条  通过ajax进行查看用户信息    -->
        <div class="providerAdd" id="userInfo" style="display:none">
               <div>
                    <label for="userCode">用户编号:</label>
                    <input type="text" name="userCode" id="v_userCode" value="" disabled>
               </div>
               <div>
                    <label for="userName">用户名称:</label>
                    <input type="text" name="userName" id="v_userName" value="" disabled>
               </div>
               <div>
                    <label for="gender">用户性别：</label>
                    <input type="text" name="gender" id="v_gender" value="" disabled> 
                </div>
                <div>
                    <label for="birthday">出生日期：</label>
                    <input type="text" name="birthday" id="v_birthday" value="" disabled> 
                </div>
                <div>
                    <label for="phone">用户电话：</label>
                    <input type="text" name="phone" id="v_phone" value="" disabled> 
                </div>
                <div>
                    <label for="address">用户地址：</label>
                    <input type="text" name="address" id="v_address" value="" disabled> 
                </div>
                <div>
                    <label for="userRoleName">用户角色：</label>
                    <input type="text" name="userRoleName" id="v_userRoleName" value="" disabled> 
                </div>
        </div>
       </div>
    </section>
